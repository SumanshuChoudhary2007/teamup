...
export default function AllDevelopersPage() {
  ...
  const filtered = developers.filter(h => {
      ...
      <div className="relative z-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <Sparkles className="w-8 h-8 text-amber-400" /> Discover Developers
          </h1>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filtered.map((developer, i) => (
              ...
              >
                <div className="relative mb-4">
                  <div className="w-24 h-24 rounded-2xl gradient-bg flex items-center justify-center text-white text-3xl font-bold shadow-xl shadow-black/20 ring-1 ring-white/10 group-hover:scale-110 transition-transform">
                    {developer.avatar_url ? (
                      <img src={developer.avatar_url} alt="" className="w-full h-full object-cover rounded-2xl" />
                    ) : (
                ...
                <p className="text-xs text-[#a78bfa] font-bold tracking-widest uppercase mb-4">{developer.role?.replace('_', ' ')}</p>
                ...
                <div className="mt-auto w-full pt-4 border-t border-white/5 flex items-center justify-center gap-4">
                  {developer.portfolio_link && (
                    <a href={developer.portfolio_link} target="_blank" className="text-[#94a3b8] hover:text-white transition-colors">
                      <Globe className="w-4 h-4" />
                    </a>
                  )}
                  <Link href={`/profile/${developer.id}`} className="text-[#a78bfa] hover:text-white text-sm font-bold flex items-center gap-1 transition-colors">
